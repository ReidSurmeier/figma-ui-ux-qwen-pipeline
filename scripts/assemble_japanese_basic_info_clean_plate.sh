...
TITLE="$REPO_ROOT/artifacts/runs/japanese-basic-info-remove-title-v002/image-01.png"
...
for donor in "$SOURCE" "$GLOBAL" "$TITLE" "$BUTTONS" "$RESOURCES" "$LEVEL_FOOTER" "$MINIMIZED"; do
  test -f "$donor"
done
...
convert "$TITLE" -filter Lanczos -resize 280x140\! -crop 280x120+0+0 +repage "$ASSET_TMP_DIR/title.png"
...
# Qwen title-repair v002 candidate 01 is selected by the executable title-tone
# gate. Use only its bounded title surface. Its globally redrawn body and the
# padded review margin are excluded from assembly.
composite_region "$ASSET_TMP_DIR/title.png" 3 2 275 15 "$ASSET_TMP_DIR/plate-3.png" "$ASSET_TMP_DIR/plate-4.png"
