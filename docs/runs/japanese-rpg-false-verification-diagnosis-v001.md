...
| Status alternate view | The alternate view is newly typeset browser text over the empty generated table, producing foreign lines and boxes rather than a source-fitting state. | The test checked only that a tabpanel contained `キャラクター情報`. |
...
| Party | Dragging Party 40 by 30 pixels moves back by exactly 40 by 30. The buttons were modeled inside the Party window despite the review requirement that they remain separate. | `windows.json` incorrectly defines Party as a `160-wide party panel plus three source action buttons`, so implementation, manifest, and tests agree with the same wrong boundary. |
