...
export function BottomBarSourceWindow({ zIndex, onActivate }: WindowProps) {
  ...
  return <SourceWindow id="bottom-bar" title="クイックスロットバー" initialPosition={{ x: 0, y: 538 }} width={600} height={21} titleWidth={80} assetRoot={assetRoot} zIndex={zIndex} onActivate={onActivate} minimizable={false} closable={false} dragHandleStyle={{ left: 0, top: 0, width: 95, height: 21 }}>
    ...
    <input className="bottom-bar-source-slider" type="range" aria-label="クイックスロット位置" data-visual-component="bottom-bar-thumb" min="0" max="100" step="1" value={position} onInput={(event) => setPosition(event.currentTarget.valueAsNumber)} onChange={(event) => setPosition(event.currentTarget.valueAsNumber)} />
    <button type="button" className="bottom-bar-source-button bottom-bar-source-button--previous" aria-label="前のスロット" aria-pressed={slot < 0} data-visual-component="bottom-bar-previous" onClick={() => setSlot((value) => value - 1)}><SourceRaster id="bottom-bar-previous" file={`${assetRoot}/components/previous`} style={{ inset: 0 }} /></button>
    <button type="button" className="bottom-bar-source-button bottom-bar-source-button--next" aria-label="次のスロット" aria-pressed={slot > 0} data-visual-component="bottom-bar-next" onClick={() => setSlot((value) => value + 1)}><SourceRaster id="bottom-bar-next" file={`${assetRoot}/components/next`} style={{ inset: 0 }} /></button>
