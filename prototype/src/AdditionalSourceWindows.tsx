...
export function CompactInfoSourceWindow({ zIndex, onActivate }: WindowProps) {
  ...
  return <SourceWindow id="compact-info" title="簡易情報" initialPosition={{ x: 568, y: 314 }} width={281} height={35} titleWidth={100} assetRoot={assetRoot} zIndex={zIndex} onActivate={onActivate} minimizable={false} closable={false}>
    ...
    <input className="compact-source-slider compact-source-slider--hp" type="range" aria-label="簡易HP" min="0" max="100" step="1" value={hp} onInput={(event) => setHp(event.currentTarget.valueAsNumber)} onChange={(event) => setHp(event.currentTarget.valueAsNumber)} />
    <input className="compact-source-slider compact-source-slider--sp" type="range" aria-label="簡易SP" min="0" max="100" step="1" value={sp} onInput={(event) => setSp(event.currentTarget.valueAsNumber)} onChange={(event) => setSp(event.currentTarget.valueAsNumber)} />
    <output className="sr-only" role="status">HP {hp} SP {sp}</output>
  </SourceWindow>;
}
