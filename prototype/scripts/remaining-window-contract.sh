...
declare -A counts=(
  [card]=14 [skills]=34 [equipment]=15 [chat]=15
...
declare -A dimensions=(
  ...
  [exchange]=280x120 [game-menu]=222x133 [party]=160x175 [quickbar]=112x94
...
for name in "${!counts[@]}"; do
  ...
  if [[ "$name" != "quickbar" && "$name" != "bottom-bar" && "$name" != "notification" ]]; then
    ...
    awk -v alpha="$(convert "$root/clean-plate.png" -alpha extract -crop "1x1+$(( $(identify -format '%w' "$root/clean-plate.png") / 2 ))+$sample_y" -format '%[fx:mean]' info:)" 'BEGIN { exit !(alpha > 0.9) }'
  fi
done
...
printf 'remaining-window-contract: PASS Qwen plates, 166 independent assets, movable source scroll thumbs, rounded pixel corners, no runtime underlay, and no donor-pink component seams\n'
